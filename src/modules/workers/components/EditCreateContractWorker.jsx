...
export const EditCreateContractWorker = ({ itemId, reFetchData }) => {
  const { data, isLoading, fetchData } = useFetchData({
    url: `/contract-workers/${itemId}`,
  });
  ...
  const { getFileInfo, isLoading: isLoadingFile } = useUploadFile({
    tableName: TABLE_NAME_FILES.contractWorkers,
  });
  ...
  return (
    <>
      ...
      <CardBase title="Contratos" async={isLoading} skeletonlines={5}>
        ...
        <Slot slot="body">
          <Table
            aria-label="Table worker's contracts"
            isStriped
