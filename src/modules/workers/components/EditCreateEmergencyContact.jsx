...
export const EditCreateEmergencyContact = ({ itemId }) => {
  ...
  const addRow = () => {
    const hasOneToCreate =
      rows.filter((row) => row.origin === "frontend").length > 0;
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      if (
        contact.name === cloneContact.name &&
        contact.phone === cloneContact.phone &&
        contact.relation === cloneContact.relation
      ) {
        ToastNotification.showWarning(
          "No se han realizado cambios en el contacto",
        );
        return;
      }
  ...
  const renderCell = useCallback(
    (item, columnKey) => {
      ...
      const cellIndex =
        ...
        columns.findIndex((col) => col.key === columnKey);
        ...
        case "phone":
          return (
            <Input
              ...
              value={cellValue}
              onChange={(e) =>
                handleInputChange(item.id, columnKey, e.target.value)
              }
          ...
          return (
            <div className="flex justify-center">
              <ButtonGroup>
                ...
                {item.origin === "frontend" && (
                  <Button
                    isIconOnly
                    color="white"
                    onClick={() => handleCreate(item.id)}
                  >
                    <span className="cursor-pointer text-primary active:opacity-50">
                      <PlusIcon />
                    </span>
                  </Button>
                )}
  ...
  return (
    <>
      ...
      >
        ...
        <Slot slot="body">
          <Table
            aria-label="Table worker's emergency contacts"
